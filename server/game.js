...
const getName = (name, gamemodeData) => {
    ...
    // Deathmatch
        clan_wars: "Clan Wars",
        ffa: "FFA",
        groups: `${gamemodeData.groups}-Player Groups`,
...
class gameServer {
    constructor(host, port, gamemode, region, webProperties, serverProperties, isfeatured, parentPort, loaderGlobal) {
        ...
        // Store the initial gamemode key so the variation system can identify this server
        this.initialGamemodeKey = gamemode.join(',');
    ...
    start(softStart = false) {
        ...
        // Schedule automatic arena close at 00:00, 06:00, 12:00, 18:00 UTC
        this.scheduleNextArenaClose();
    }

    scheduleNextArenaClose() {
        if (this._arenaCloseTimer) clearTimeout(this._arenaCloseTimer);
        const SIX_HOURS = 6 * 60 * 60 * 1000;
        const now = Date.now();
        const midnight = new Date();
        midnight.setUTCHours(0, 0, 0, 0);
        const msSinceMidnight = now - midnight.getTime();
        const msToNext = SIX_HOURS - (msSinceMidnight % SIX_HOURS);
        this._arenaCloseTimer = setTimeout(() => this.closeArena(), msToNext);
    }
    ...
    // Build importedRoom grid + wrap tiles as tileEntity — does NOT run tile INITs.
    // Called by setRoom() for first start, and by _applyVariation() for room changes.
    _buildRoomGrid() {
        for (let filename of Config.room_setup) {
            const roomPath = require.resolve(`./game/roomSetup/rooms/${filename}.js`);
            ...
            for (let y = 0; y < Config.roomHeight; y++) {
                for (let x = 0; x < Config.roomWidth; x++) {
                    if (this.importedRoom[y] == null) {
                        this.importedRoom[y] = this.currentRoom[y].slice();
        ...
        for (let y in this.room.setup) {
            for (let x in this.room.setup[y]) {
                this.room.setup[y][x] = new tileEntity(this.room.setup[y][x], { x, y }, this);
            }
        }
    }

    // Set up the room (first start only — runs tile INITs after building the grid)
    setRoom() {
        this._buildRoomGrid();
        for (let y in this.room.setup) {
            for (let x in this.room.setup[y]) {
    ...
    _getVariationPool() {
        try {
            const variations = require('./game/gamemodes/variations.js');
            if (variations[this.initialGamemodeKey]) return variations[this.initialGamemodeKey];
            // Subset match: server gamemodes ⊇ variation key modes (order-insensitive)
            for (const key of Object.keys(variations)) {
                if (key.split(',').every(m => this.gamemode.includes(m)))
    ...
    _applyVariation(variation) {
        Config.room_setup = ['room_default'];
        let overrideRoom = true;
        for (const modeName of variation.modes) {
            try {
                const mode = require(`./game/gamemodes/config/${modeName}.js`);
                for (const key in mode) {
                    if (key === 'do_not_override_room') overrideRoom = mode[key];
                    else if (key === 'room_setup') {
                        if (!overrideRoom) Config.room_setup = mode[key];
                        else Config.room_setup.push(...mode[key]);
                    } else Config[key] = mode[key];
                }
            } catch (e) { util.log(`Variation: missing config '${modeName}'`); }
        }
        for (const key in variation.overrides) Config[key] = variation.overrides[key];

        // Always rebuild so room files re-evaluate with the new Config (teams, mode, etc.)
        this.importedRoom = [];
        this._buildRoomGrid();
    ...
    // Instant restart — no arena closers, just wipes immediately
    stopArena() {
        if (this.arenaClosed) return;
        util.log(`Arena stop (instant) requested.`);
        this.socketManager.broadcast("Arena restarting...");
        this.close(null);
    }
    ...
    close(spawnTimeout) {
        if (this._arenaCloseTimer) { clearTimeout(this._arenaCloseTimer); this._arenaCloseTimer = null; }
    ...
    onEnd() {
        ...
        const pool = this._getVariationPool();
        if (pool) {
            let variation;
            if (this._forcedVariation) {
                const forced = this._forcedVariation.toLowerCase();
                variation = pool.find(v => v.name.toLowerCase() === forced) || ran.choose(pool);
                this._forcedVariation = null;
            } else {
                variation = ran.choose(pool);
            }
            this._applyVariation(variation);
        } else {
            this._forcedVariation = null;
        }
        if (this.parentPort) this.parentPort.postMessage([false, this.getInfo()]);
        this.start(true);
    }
