...
class io_siegeAI extends IO {
    constructor(body, opts = {}) {
        super(body);
        this.enabled = true;
        // Random orbit parameters per boss so they spread out and don't all move identically
        this.orbitAngle = Math.random() * Math.PI * 2;
        this.orbitRadius = 280 + Math.random() * 140; // 280–420 units from sanctuary centre
        this.orbitSpeed = -(0.012 + Math.random() * 0.010); // always clockwise
        this.target = null;
        this.retargetTimer = 0;
    }
    _pickSanctuary() {
        const siege = global.gameManager.gamemodeManager?.gameSiege;
        if (!siege) return null;
        // Only target sanctuaries that are still alive and belong to the player team
        const alive = siege.sanctuaries.filter(s => !s.isDead() && s.team !== TEAM_ENEMIES);
        if (!alive.length) return null;
        // Pick the nearest one so bosses naturally spread across multiple sanctuaries
        let best = null, bestDist = Infinity;
        for (const s of alive) {
            const d = (s.x - this.body.x) ** 2 + (s.y - this.body.y) ** 2;
    ...
    think(input) {
        ...
        if (tile && tile.name === "stopAI") this.enabled = false;
        if (!this.enabled) return;

        // Re-evaluate target periodically or when it disappears
        if (--this.retargetTimer <= 0 || !this.target || this.target.isDead()) {
            this.target = this._pickSanctuary();
            this.retargetTimer = 45; // ~1.5 s at 30 tps
        }

        // No sanctuary alive yet → fall back to room centre
        if (!this.target) return { goal: global.gameManager.room.center };

        this.orbitAngle += this.orbitSpeed;
        return {
            goal: {
                x: this.target.x + Math.cos(this.orbitAngle) * this.orbitRadius,
...
// Follows the ally player with the highest score — rogues are TEAM_BLUE allies in siege
class io_huntHighestScore extends IO {
    constructor(body) {
        super(body);
        this.retargetTimer = 0;
        this.target = null;
    ...
    think() {
        if (--this.retargetTimer <= 0) {
            this.retargetTimer = 30;
            let best = null, bestScore = -Infinity;
            for (const [, e] of entities) {
                if (e.type !== 'tank' || !e.isPlayer || e.isDead() || e.invuln || e.godmode) continue;
                if (e.team !== this.body.team) continue; // follow highest-score ally
                if (e.skill.score > bestScore) { bestScore = e.skill.score; best = e; }
            }
            this.target = best;
        }
        if (!this.target || this.target.isDead()) return;
        // Only set goal (movement); combat controllers handle targeting enemies independently
        return { goal: { x: this.target.x, y: this.target.y }, main: true };
    }
}
...
let ioTypes = {
    ...
    huntHighestScore: io_huntHighestScore,
