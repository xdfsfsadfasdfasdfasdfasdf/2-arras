...
class socketManager {
        ...
        switch (m.shift()) {
            ...
            case "U": {
                ...
                if (player.body != null) {
                    ...
                    // When a player picks a boss via the dev menu, switch them to the enemy team
                    // so they fight alongside NPC bosses and against player tanks.
                    const isBossClass = player.body.type === "miniboss";
                    if (isBossClass && socket.status.bossMenuOriginalTeam === undefined) {
                        socket.status.bossMenuOriginalTeam = player.body.team;
                        player.body.team = TEAM_ENEMIES;
                        player.body.isBoss = true;
                        player.body.refreshBodyAttributes();
                        player.body.sendMessage("You are now on the boss team! Use $team to switch sides.");
                    } else if (!isBossClass && socket.status.bossMenuOriginalTeam !== undefined) {
                        player.body.team = socket.status.bossMenuOriginalTeam;
                        player.body.isBoss = false;
                        player.body.refreshBodyAttributes();
                        delete socket.status.bossMenuOriginalTeam;
                        player.body.sendMessage("You have returned to your original team.");
                    }
                }
            } break;
            case "x": {
            ...
            case "SS": { // spectator speed adjustment
                if (!socket.status.isSpectating || !player.body) break;
                let dir = m[0];
                if (dir !== 1 && dir !== -1) break;
                if (!socket.status.spectatorSpeed) socket.status.spectatorSpeed = 8;
                socket.status.spectatorSpeed = Math.max(1, Math.min(50, socket.status.spectatorSpeed + dir * 3));
                player.body.define({ BODY: { SPEED: socket.status.spectatorSpeed } });
            } break;
            default: {
                console.log(m)
    ...
    update(gui) {
        ...
        gui.bodyid = b.id;
        let dailyTank = null;
        // Ensure teamColor is always set before being read
        if (!gui.master.teamColor) {
            gui.master.teamColor = new Color(!Config.random_body_colors && (Config.groups || (Config.mode === 'ffa' && !Config.tag)) ? 10 : global.getTeamColor(b.team)).compiled;
        }
        // Update most things
