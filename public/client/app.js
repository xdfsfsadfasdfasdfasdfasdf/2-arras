...
(async function (util, global, config, Canvas, color, gameDraw, socketStuff) {
    ...
    window.onload = async () => {
        ...
        loadSettings();
        // Sync custom theme maker now that optColors is loaded from localStorage
        global.syncCustomThemeMaker && global.syncCustomThemeMaker();
        // Keybinds stuff
        getKeybinds();
    ...
    // Options sidebar — works on home menu and in-game overlay.
    function initSidebar() {
        ...
        function openSidebar() {
            ...
            openBtn.classList.add("sidebar-btn-hidden");
            if (global.optionsMenu_Anim) global.optionsMenu_Anim.isOpened = true;
        }
        function closeSidebar() {
            sidebar.classList.remove("open");
            overlay.classList.remove("visible");
            openBtn.classList.remove("sidebar-btn-hidden");
        ...
        // Apply settings immediately when any sidebar control changes (important for in-game use).
        const liveSettingIds = [
            "optSharpEdges", "optSlowerFOV", "optPredictive", "optFancy",
            "optLowResolution", "coloredHealthbars", "smoothCamera", "optColors",
            "optPointy", "optCurvyTraps", "optInterpolation", "optLerpAnim",
            "optOptimizeMode", "optCenterMinimap", "optBorders", "optNoGrid",
            "optColoredNest", "optRenderKillbar", "separatedHealthbars",
            "autoLevelUp", "optMobile", "optRenderGui", "optRenderLeaderboard",
            "optRenderUpgrades", "optRenderMinimap", "optRenderNames",
            "optRenderHealth", "optRenderScores", "optRenderPlayerBars",
            "optReducedInfo", "showCrosshair", "showJoystick", "optFullHD",
            "optUiScale", "optIncognitoMode",
        ];
        for (const id of liveSettingIds) {
            const el = document.getElementById(id);
            if (!el) continue;
            el.addEventListener("change", () => {
                util.submitToLocalStorage(id);
                loadSettings();
                if (id === "optLowResolution") resizeEvent();
            });
        }
    };
            ...
            buttonTabs.children[g].addEventListener("click", () => {
                e !== g &&
                    (buttonTabs.children[e].classList.remove("active"),
                        ...
                        (tabOptions[g].style.display = ""), // Let CSS class control display type
                        (e = g))
            });
    }
    ...
    // Custom theme maker — lets users pick any color for any game element.
    function initCustomThemeMaker() {
        const CTM_KEYS = [
            // Background & UI
            { key: "white",    label: "Arena BG"  },
            { key: "guiblack", label: "Grid"       },
            { key: "grey",     label: "Barrels"    },
            { key: "guiwhite", label: "Text"       },
            { key: "black",    label: "Borders"    },
            // Health / Shield
            { key: "lgreen",   label: "Health Bar" },
            { key: "teal",     label: "Shield Bar" },
            // Team colors
            { key: "blue",     label: "Blue Team"  },
            { key: "green",    label: "Green Team" },
            { key: "red",      label: "Red Team"   },
            { key: "magenta",  label: "Purple Team"},
            { key: "mustard",  label: "Yellow Team"},
            { key: "tangerine",label: "Orange Team"},
            { key: "brown",    label: "Brown Team" },
            { key: "cyan",     label: "Cyan Team"  },
            { key: "yellow",   label: "Neutral"    },
            { key: "dgrey",    label: "Rogue"      },
            // Polygon colors
            { key: "vlgrey",   label: "Eggs"       },
            { key: "gold",     label: "Squares"    },
            { key: "orange",   label: "Triangles"  },
            { key: "purple",   label: "Pentagons"  },
            { key: "aqua",     label: "Hexagons"   },
            { key: "heptagon", label: "Heptagons"  },
            { key: "pink",     label: "Crashers"   },
            { key: "lavender", label: "Nest"       },
            { key: "lgrey",    label: "Walls"      },
        ];

        const optColorsEl  = document.getElementById("optColors");
        const optCustomEl  = document.getElementById("optCustom");
        const ctmPanel     = document.getElementById("customThemeMaker");
        const ctmName      = document.getElementById("ctm_name");
        const ctmAuthor    = document.getElementById("ctm_author");
        const ctmBorder    = document.getElementById("ctm_border");
        const ctmBorderLbl = document.getElementById("ctm_border_label");
        if (!ctmPanel) return;

        function generateAndApply() {
            const name   = ctmName.value.trim()  || "My Theme";
            const author = ctmAuthor.value.trim() || "";
            const border = (ctmBorder.value / 100).toFixed(2);
            const content = { paletteSize: 20, border: parseFloat(border) };
            for (const { key } of CTM_KEYS) {
                const el = document.getElementById(`ctm_${key}`);
                if (el) content[key] = el.value;
            }
            optCustomEl.value = JSON.stringify({ name, author, content });
            optColorsEl.value = "custom";
            util.submitToLocalStorage("optCustom");
            util.submitToLocalStorage("optColors");
            loadSettings();
        }

        function loadColorsFromTheme(themeName) {
            let src;
            if (themeName === "custom") {
                const parsed = parseTheme(optCustomEl.value, false);
                src = (parsed && parsed.content) ? parsed.content : colors["normal"];
                if (ctmName && parsed) ctmName.value = parsed.name !== "Unknown Theme" ? parsed.name : "";
                if (ctmAuthor && parsed) ctmAuthor.value = parsed.author || "";
                const borderPct = src.border !== undefined ? Math.round(src.border * 100) : 65;
                ctmBorder.value = borderPct;
                if (ctmBorderLbl) ctmBorderLbl.textContent = borderPct + "%";
            } else {
                src = colors[themeName] || colors["normal"];
                ctmBorder.value = Math.round((src.border || 0.65) * 100);
                if (ctmBorderLbl) ctmBorderLbl.textContent = ctmBorder.value + "%";
            }
            for (const { key } of CTM_KEYS) {
                const el = document.getElementById(`ctm_${key}`);
                if (el && src[key] && /^#[0-9a-fA-F]{6}$/.test(src[key])) el.value = src[key];
            }
        }

        function syncUI(val) {
            const isCustom = val === "custom";
            ctmPanel.style.display    = isCustom ? "block" : "none";
            optCustomEl.style.display = "none"; // string field hidden; maker generates it
        }

        optColorsEl.addEventListener("change", () => {
            syncUI(optColorsEl.value);
            if (optColorsEl.value === "custom") loadColorsFromTheme("custom");
            else loadColorsFromTheme(optColorsEl.value);
        });

        ctmBorder.addEventListener("input", () => {
            if (ctmBorderLbl) ctmBorderLbl.textContent = ctmBorder.value + "%";
            generateAndApply();
        });
        ...
        for (const { key } of CTM_KEYS) {
            const el = document.getElementById(`ctm_${key}`);
            if (el) el.addEventListener("input", generateAndApply);
        }
        ctmName.addEventListener("input", generateAndApply);
        ctmAuthor.addEventListener("input", generateAndApply);

        // Expose sync so window.onload can call it after localStorage values are restored
        global.syncCustomThemeMaker = () => {
            syncUI(optColorsEl.value);
            if (optColorsEl.value === "custom") loadColorsFromTheme("custom");
        };
    }
    ...
    const iconColorOrder = [10, 11, 12, 15, 3, 6, 13, 2, 14, 4, "heptagon", 5, 1, 0, 25, 26, 27, 28];
    function getIconColor(colorIndex) {
        return iconColorOrder[colorIndex % 18].toString();
    }
    ...
    function drawOptionsMenu() {
        // The options panel is now an HTML overlay sidebar (#optionsSidebar).
        // The canvas button is no longer drawn; the HTML #sidebarOpenBtn handles toggling.
        return;
    ...
    const drawErrorScreen = (err) => {
        let ratio = util.getScreenRatio();
        scaleScreenRatio(ratio, true);
        clearScreen(gameDraw.mixColors(color.black, color.guiblack, 0.3), 0.25, ctx[2]);
        drawText("Client error detected!", global.screenWidth / 2, global.screenHeight / 2 - 30, 30, color.red, "center");
        if (err) drawText(String(err).slice(0, 150), global.screenWidth / 2, global.screenHeight / 2 + 5, 13, color.guiwhite, "center");
    ...
    function animloop(tick) {
        ...
        if (global.gameStart) {
            ...
            if (global.isSpectating) renderfov *= (global.spectatorFovMult ?? 1);
